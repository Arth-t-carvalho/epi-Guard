...
function openExportModal() {
    ...
    listContainer.innerHTML = `
        <div class="employee-empty info">
            ...
            <span>${window.I18N['Nenhum setor selecionado. Por favor, escolha um setor acima para carregar a lista.']}</span>
        </div>
    `;
...
async function loadEmployeesForExport(sectorId) {
    ...
    listContainer.innerHTML = `
        <div class="employee-loading">
            <i class="fa-solid fa-spinner fa-spin"></i> ${window.I18N['Carregando funcionários...']}
        </div>
    `;
    ...
    try {
        ...
        if (result.success && result.data.length > 0) {
            ...
            listContainer.innerHTML = `
                <div class="employee-empty">
                    ...
                    <span>${window.I18N['Nenhum funcionário encontrado neste setor.']}</span>
                </div>
            `;
            formatStep.style.display = 'none';
        }
        ...
        listContainer.innerHTML = `
            <div class="employee-empty error">
                ...
                <span>${window.I18N['Erro ao carregar dados. Tente novamente.']}</span>
            </div>
        `;
    }
}
...
function updateSelectedCount() {
    ...
    if (el) {
        el.textContent = `${checked} ${window.I18N['de']} ${total} ${window.I18N['selecionados']}`;
    }
...
function processExport(format) {
    ...
    if (selectedEmployees.length === 0) {
        alert(window.I18N['Selecione pelo menos um funcionário para exportar.']);
        return;
    }
    ...
    btn.querySelector('.btn-text').innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> ${window.I18N['Gerando %s...'].replace('%s', format.toUpperCase())}`;
    btn.disabled = true;
    btn.style.opacity = '0.7';

    // Simulação (substituir por chamada real ao backend)
    setTimeout(() => {
        btn.querySelector('.btn-text').innerHTML = `<i class="fa-solid fa-check"></i> ${window.I18N['Concluído!']}`;
        btn.style.opacity = '1';
        
        setTimeout(() => {
            btn.querySelector('.btn-text').innerHTML = originalText;
            btn.disabled = false;
            alert(window.I18N['Relatório %s gerado com sucesso para %d funcionário(s).'].replace('%s', format.toUpperCase()).replace('%d', selectedEmployees.length));
        }, 1500);
    }, 2500);
}
