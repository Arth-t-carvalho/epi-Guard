...
if (!window._notificationsGlobalsInitialized) {
    ...
    function renderNotificationDetail(notif) {
        ...
        detailDiv.innerHTML = `
            <div class="notif-back-btn" style="color:var(--primary);font-weight:700;font-size:13px;cursor:pointer;display:flex;align-items:center;gap:6px;width:fit-content;padding:4px 0;user-select:none;">
                <i class="fa-solid fa-arrow-left"></i> ${window.I18N?.labels?.back || 'Back'}
            </div>
            
            <div class="notif-detail-card">
                <div style="background:var(--primary);color:white;width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px;">
                    <i class="fa-solid fa-triangle-exclamation"></i>
                </div>
                <h3 style="margin:4px 0 0 0;color:var(--text-main);font-size:17px;font-weight:800;">${nome}</h3>
                <span style="color:var(--primary);font-weight:700;font-size:12px;letter-spacing:0.5px;">${window.I18N?.labels?.infraction_upper || 'INFRACTION'}</span>
            </div>

            <div style="display:flex;flex-direction:column;gap:10px;margin-top:4px;">
                <div class="notif-info-row">
                    <span class="label">${window.I18N?.labels?.sector || 'Department'}</span>
                    <span class="value">${setor}</span>
                </div>
                <div class="notif-info-row">
                    <span class="label">${window.I18N?.labels?.missing_ppe || 'Missing PPE'}</span>
                    <span class="value" style="color:var(--primary);">${epi}</span>
                </div>
                <div class="notif-info-row">
                    <span class="label">${window.I18N?.labels?.date || 'Date'}</span>
                    <span class="value">${dataStr}</span>
                </div>
                <div class="notif-info-row" style="border-bottom:none;">
                    <span class="label">${window.I18N?.labels?.time || 'Time'}</span>
                    <span class="value">${horaStr}</span>
                </div>
            </div>

            <button class="notif-goto-infractions" style="margin-top:8px;width:100%;background:var(--primary);color:white;padding:12px;border:none;border-radius:8px;font-weight:700;font-size:13px;cursor:pointer;display:flex;justify-content:center;align-items:center;gap:6px;transition:all 0.2s;">
        ...
        gotoBtn.addEventListener('click', function(e) {
            ...
            const targetUrl = `${window.BASE_PATH || ''}/infractions?funcionario_id=${notif.funcionario_id}&highlight=${notif.id}`;
    ...
    function renderNotification(notif) {
        ...
        div.innerHTML = `
            <div style="background:var(--primary-light);color:var(--primary);width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            ...
            <div style="flex:1;display:flex;flex-direction:column;gap:3px;min-width:0;padding-right:20px;">
                <strong style="font-size:13px;color:var(--text-main);font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${nome}</strong>
                <div style="display:flex;align-items:center;gap:4px;color:var(--text-muted);font-size:12px;font-weight:600;">
                ...
                <span style="font-size:11px;color:var(--text-muted);font-weight:600;opacity:0.8;">${timeStr}</span>
            </div>
    ...
    function showEpiNotification(data) {
        ...
        toast.innerHTML = `
            <div class="toast-icon-wrapper"><i class="fa-solid fa-triangle-exclamation"></i></div>
    ...
    window.showAlert = function(title, message, type = 'info') {
        return new Promise((resolve) => {
            ...
            if (!modal || !titleEl || !msgEl || !iconEl || !iconWrapper) {
                ...
                // Fallback mais limpo
                const fallbackMessage = (title && title !== 'Aviso') ? `${title}: ${message}` : message;
                window._originalAlert(fallbackMessage);
                resolve(true);
                return;
            }

            titleEl.textContent = title || window.I18N?.labels?.alert || 'Alerta';
    ...
    window.showConfirm = function(title, message, type = 'warning') {
        return new Promise((resolve) => {
            ...
            if (!modal || !titleEl || !msgEl || !iconEl || !iconWrapper) {
                const res = window._originalConfirm(`${title}\n\n${message}`);
                resolve(res);
                return;
            }

            titleEl.textContent = title || window.I18N?.labels?.confirm_title || 'Confirmação';
            msgEl.textContent = message;
    ...
    // --- OVERRIDE GLOBAL DE ALERT E CONFIRM ---
    if (!window._nativeOverridesApplied) {
        window._originalAlert = window.alert;
        window._originalConfirm = window.confirm;
        
        window.alert = function(message) {
            // Se for um objeto (erro), tenta pegar a mensagem
            const msg = (typeof message === 'object' && message !== null) ? (message.message || JSON.stringify(message)) : message;
            return window.showAlert(window.I18N?.labels?.alert || 'Aviso', msg, 'info');
        };

        window.confirm = function(message) {
            return window.showConfirm(window.I18N?.labels?.confirm_title || 'Confirmação', message, 'warning');
        };

        window._nativeOverridesApplied = true;
    }
}
