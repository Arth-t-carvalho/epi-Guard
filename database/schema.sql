...
CREATE TABLE IF NOT EXISTS usuarios (
  ...
  turno ENUM('MANHA','TARDE','NOITE','INTEGRAL'),
  status ENUM('ATIVO','INATIVO') NOT NULL DEFAULT 'ATIVO',
  preferencia_grafico ENUM('bar','line','area') DEFAULT 'bar',
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS maquinas (
  ...
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_maquina_setor (setor_id),
...
-- 2. Adicionar filial_id nas tabelas relacionadas
ALTER TABLE ocorrencias ADD COLUMN IF NOT EXISTS filial_id INT(11) AFTER funcionario_id;
ALTER TABLE funcionarios ADD COLUMN IF NOT EXISTS filial_id INT(11) AFTER id;
ALTER TABLE setores ADD COLUMN IF NOT EXISTS filial_id INT(11) AFTER id;

-- 3. Inserir filiais iniciais
INSERT INTO filiais (id, nome, cidade, estado, status) VALUES 
(1, 'Aparecida do Taboado', 'Aparecida do Taboado', 'MS', 'ATIVO'),
(2, 'Votuporanga', 'Votuporanga', 'SP', 'ATIVO')
ON DUPLICATE KEY UPDATE nome=VALUES(nome);

-- 4. Vincular dados existentes à filial de Aparecida do Taboado (ID 1)
UPDATE ocorrencias SET filial_id = 1 WHERE filial_id IS NULL;
UPDATE funcionarios SET filial_id = 1 WHERE filial_id IS NULL;
UPDATE setores SET filial_id = 1 WHERE filial_id IS NULL;

-- -----------------------------------------------------
-- 12. UPDATE SCHEMA V2 (USUARIOS)
-- -----------------------------------------------------
-- 1. Garante que a coluna 'filial_id' existe (necessária para segmentação por filial)
ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS filial_id INT DEFAULT 1 AFTER id;

-- 2. Garante que a coluna 'preferencia_grafico' existe (necessária para o dashboard customizado)
ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS preferencia_grafico ENUM('bar', 'line', 'area') DEFAULT 'bar' AFTER atualizado_em;

-- 3. Garante que a coluna 'status' existe (necessária para filtros de login)
ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS status ENUM('ATIVO', 'INATIVO') DEFAULT 'ATIVO' AFTER turno;

-- 4. Atualiza registros antigos que podem estar com valores nulos
UPDATE usuarios SET filial_id = 1 WHERE filial_id IS NULL;
UPDATE usuarios SET preferencia_grafico = 'bar' WHERE preferencia_grafico IS NULL;
