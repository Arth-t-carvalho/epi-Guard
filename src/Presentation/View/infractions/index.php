                ...
                <?php foreach ($infractions as $infraction): ?>
                    <div class="infraction-card<?= !empty($infraction['favorito']) ? ' is-bookmarked' : '' ?><?= ($highlightId == $infraction['id']) ? ' highlight-notification' : '' ?>"
                        ...
                        <div class="card-image-box">
                            <?php
                            // Prioridade: Foto da Evidência -> Foto do Funcionário -> Avatar Padrão
                            if (!empty($infraction['evidencia_foto'])) {
                                $photoPath = BASE_PATH . '/public/uploads/' . ltrim(str_replace('\\', '/', $infraction['evidencia_foto']), '/');
                                $isEvidence = true;
                            } elseif (!empty($infraction['funcionario_foto'])) {
                                $photoPath = BASE_PATH . '/public/uploads/' . ltrim(str_replace('\\', '/', $infraction['funcionario_foto']), '/');
                                $isEvidence = false;
                            } else {
                                $photoPath = BASE_PATH . '/assets/img/default-avatar.png';
                                $isEvidence = false;
                            }
                            ?>
                            <img src="<?= $photoPath ?>" alt="<?= htmlspecialchars($infraction['funcionario_nome']) ?>"
                                class="<?= $isEvidence ? 'card-evidence-photo' : 'card-employee-photo' ?>" 
                                onerror="this.src='<?= BASE_PATH ?>/assets/img/default-avatar.png'"
                                style="<?= $isEvidence ? 'object-fit: cover; width: 100%; height: 100%;' : '' ?>">
                                <span
                        ...
                        <div class="card-content-premium">
                            ...
                            <div class="card-footer-premium">
                                ...
                                $args = json_encode([
                                    ...
                                    !empty($infraction['evidencia_foto']) ? BASE_PATH . '/public/uploads/' . ltrim(str_replace('\\', '/', $infraction['evidencia_foto']), '/') : '',
                                    $infraction['id'],
