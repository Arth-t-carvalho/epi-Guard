<?php require_once __DIR__ . '/../../../Infrastructure/Bootstrap.php'; ?>
<!DOCTYPE html>
<html lang="pt-br">
...
<head>
    ...
    <script>
        (function () {
            const savedTheme = localStorage.getItem('Facchini-theme');
    ...
    <link rel="stylesheet"
        href="<?= BASE_PATH ?>/assets/css/global.css?v=<?= @filemtime(BASE_DIR . '/assets/css/global.css') ?>">
    <link rel="stylesheet"
        href="<?= BASE_PATH ?>/assets/css/sidebar.css?v=<?= @filemtime(BASE_DIR . '/assets/css/sidebar.css') ?>">
    <link rel="stylesheet" href="<?= BASE_PATH ?>/assets/css/modal/modalBase.css?v=<?= @filemtime(BASE_DIR . '/assets/css/modal/modalBase.css') ?>">
    <link rel="stylesheet" href="<?= BASE_PATH ?>/assets/css/custom-select.css?v=<?= @filemtime(BASE_DIR . '/assets/css/custom-select.css') ?>">
...
<body data-base-path="<?= BASE_PATH ?>">
    ...
    <div id="notification-container" class="notification-container"
        style="position:fixed;top:20px;right:20px;z-index:999999999;display:flex;flex-direction:column;gap:15px;max-width:380px;pointer-events:none;">
    </div>
    ...
    <script id="i18n-bridge">
        window.I18N = {
            ...
            labels: {
                ...
                select_an_option: <?= json_encode(__('select_an_option')) ?>,
                tap_item_below: <?= json_encode(__('tap_item_below')) ?>,
                select_department: <?= json_encode(__('select_department')) ?>,
                choose_area: <?= json_encode(__('choose_area')) ?>,
                select_employee: <?= json_encode(__('select_employee')) ?>,
                choose_person: <?= json_encode(__('choose_person')) ?>,
                select_position: <?= json_encode(__('select_position')) ?>,
                choose_hierarchy: <?= json_encode(__('choose_hierarchy')) ?>,
                select_ppe: <?= json_encode(__('select_ppe')) ?>,
                choose_ppe: <?= json_encode(__('choose_ppe')) ?>,
                select_reason: <?= json_encode(__('select_reason')) ?>,
                choose_justification: <?= json_encode(__('choose_justification')) ?>,
                select_type: <?= json_encode(__('select_type')) ?>,
                choose_type_desc: <?= json_encode(__('choose_type_desc')) ?>,
                platform_language: <?= json_encode(__('platform_language')) ?>,
                change_display_lang: <?= json_encode(__('change_display_lang')) ?>
            },
            epi_colors: {
                'capacete': <?= json_encode(__('Capacete')) ?>,
                'oculos': <?= json_encode(__('Óculos')) ?>,
    ...
    <script src="<?= BASE_PATH ?>/assets/js/custom-select.js?v=<?= @filemtime(BASE_DIR . '/assets/js/custom-select.js') ?>"></script>
    <script
        src="<?= BASE_PATH ?>/assets/js/notifications.js?v=<?= @filemtime(BASE_DIR . '/assets/js/notifications.js') ?>"></script>
    <?= $extraScripts ?? '' ?>
