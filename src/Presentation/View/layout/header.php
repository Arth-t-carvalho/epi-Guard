...
$userEmail = $_SESSION['user_email'] ?? 'arthur@epiguard.com';
...
<header class="header">
    ...
    <div class="page-title">
        <div id="welcome-truck-container" class="welcome-container">
            ...
            <p class="welcome-text"><?= __('Olá') ?> <?= explode(' ', $userName)[0] ?>, <?= __('bem-vindo de volta!') ?></p>
        </div>
    </div>
    ...
    <div class="header-actions">
        ...
        <button class="header-export-btn" id="headerExportDashboardBtn" style="display: none;" onclick="exportDashboardData && exportDashboardData()">
        ...
        <!-- Idioma -->
        <?php $currentLang = $_COOKIE['epiguard-lang'] ?? 'pt-br'; ?>
        <button class="header-lang-btn" onclick="changeLanguage('<?= $currentLang === 'en' ? 'pt-br' : 'en' ?>')" title="<?= $currentLang === 'en' ? 'Mudar para Português' : 'Switch to English' ?>">
            <?= $currentLang === 'en' ? 'EN' : 'BR' ?>
        </button>

        <!-- Configurações -->
        <button class="header-icon-btn" onclick="window.location.href='<?= BASE_PATH ?>/settings'" title="<?= __('Configurações') ?>">
        ...
        <div style="position: relative;" id="profileContainer">
            ...
            <div class="instructor-card" id="userProfileModal" style="position: absolute; top: calc(100% + 5px); right: 0; width: 100%; z-index: 100000; padding: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); display: none;">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 12px; margin-bottom: 20px; border-bottom: 1px solid var(--border); padding-bottom: 15px;">
                    <div class="user-avatar" style="width: 60px; height: 60px; font-size: 24px; background: var(--primary);">
                        <?= $initials ?>
                    </div>
                    <div style="text-align: center;">
                        <h4 style="margin: 0; font-size: 16px; color: var(--secondary);"><?= htmlspecialchars($userName) ?></h4>
                        <span style="font-size: 12px; color: var(--text-muted);"><?= htmlspecialchars($displayRole) ?></span>
                    </div>
                </div>
                
                <div style="width: 100%; display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px;">
                    <div class="detail-row" style="padding: 8px 0; border-bottom: 1px solid var(--border); font-size: 13px;">
                        <span class="detail-label"><?= __('Setor') ?></span>
                        <span class="detail-value"><?= htmlspecialchars($_SESSION['user_setor'] ?? __('Não atribuído')) ?></span>
                    </div>
                    <div class="detail-row" style="padding: 8px 0; border: none; font-size: 13px;">
                        <span class="detail-label"><?= __('E-mail') ?></span>
                        <span class="detail-value" style="font-size: 11px; word-break: break-all;"><?= htmlspecialchars($userEmail) ?></span>
                    </div>
                </div>
                
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    <a href="<?= BASE_PATH ?>/logout" class="btn-liquid" style="padding: 10px; font-size: 12px; text-decoration: none; display: block; text-align: center;">
                        <span class="btn-text">
