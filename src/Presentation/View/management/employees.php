...
<header class="page-header">
    ...
    <div class="header-actions">
        <a href="<?= BASE_PATH ?>/management/template-pdf" target="_blank" class="btn-secondary" style="text-decoration: none; display: inline-flex; align-items: center; gap: 8px; margin-right: 10px;">
            <i class="fa-solid fa-file-pdf"></i> <?= __('Modelo PDF') ?>
        </a>
...
<script>
    ...
    async function saveEmployee() {
        ...
        if (!nome || !setor_id) return showAlert('<?= __('Aviso') ?>', '<?= __('Preencha todos os campos.') ?>', 'warning');
        ...
        try {
            ...
            if (result.success) {
                closeEmployeeModal();
                loadEmployeesTable();
                showAlert('<?= __('Sucesso') ?>', '<?= __('Dados salvos com sucesso!') ?>', 'success');
            } else {
                showAlert('<?= __('Erro') ?>', result.error, 'error');
            }
        } catch (error) {
            console.error('Erro ao salvar:', error);
            showAlert('<?= __('Erro') ?>', '<?= __('Erro na conexão.') ?>', 'error');
        } finally {
            btn.disabled = false;
    ...
    async function deleteEmployee(id) {
        const confirmed = await showConfirm(
            ...
            '<?= __('Deseja realmente excluir este aluno?') ?>',
            'error'
        );
        if (!confirmed) return;
