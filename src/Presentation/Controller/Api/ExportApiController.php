...
class ExportApiController
{
    ...
    private function getWorstDayOfWeek(int $year, int $activeFilial): array
    {
        ...
        $query = "
            SELECT (DAYOFWEEK(o.data_hora) - 1) as dia_semana, COUNT(o.id) as total_infracoes
            FROM ocorrencias o
            WHERE o.tipo = 'INFRACAO' AND YEAR(o.data_hora) = ? AND o.filial_id = ?
            GROUP BY dia_semana
            ORDER BY total_infracoes DESC
    ...
    public function infractionsReport()
    {
        ...
        try {
            ...
            if ($periodo === 'hoje') {
                $dateFilter = " AND DATE(o.data_hora) = CURRENT_DATE";
            } elseif ($periodo === 'semana') {
                $dateFilter = " AND YEARWEEK(o.data_hora, 3) = YEARWEEK(CURRENT_DATE, 3)";
            } elseif ($periodo === 'mes') {
                $dateFilter = " AND MONTH(o.data_hora) = MONTH(CURRENT_DATE) AND YEAR(o.data_hora) = YEAR(CURRENT_DATE)";
            } elseif ($periodo === 'personalizado' && !empty($dateFrom) && !empty($dateTo)) {
                $dateFilter = " AND DATE(o.data_hora) BETWEEN ? AND ?";
                $subParams[] = $dateFrom;
                $subParams[] = $dateTo;
            }
            ...
            $query = "
                SELECT 
                    ...
                    (SELECT e.nome 
                     ...
                     LIMIT 1) as natureza,
                    (SELECT AVG(TIMESTAMPDIFF(MINUTE, o3.data_hora, COALESCE((SELECT MIN(data_hora) FROM acoes_ocorrencia ao WHERE ao.ocorrencia_id = o3.id), NOW())))
            ...
            $detailedQuery = "
                ...
                JOIN funcionarios f ON o.funcionario_id = f.id
                LEFT JOIN setores s ON f.setor_id = s.id
                WHERE o.funcionario_id IN ($placeholders) AND o.tipo = 'INFRACAO' AND (o.oculto = 0 OR o.oculto IS NULL) $dateFilter
