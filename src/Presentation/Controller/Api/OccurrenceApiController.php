...
class OccurrenceApiController
{
    ...
    public function calendar()
    {
        ...
        $query = "
            ...
            WHERE MONTH(o.data_hora) = ? AND YEAR(o.data_hora) = ? AND o.filial_id = ?
        ";
        ...
        if (!empty($sectorIds)) {
            $types = 'iii' . str_repeat('i', count($sectorIds));
            $params = array_merge([$month, $year, $activeFilial], $sectorIds);
            $stmt->bind_param($types, ...$params);
        } else {
            $stmt->bind_param('iii', $month, $year, $activeFilial);
        }
