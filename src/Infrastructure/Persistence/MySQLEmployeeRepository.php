...
class MySQLEmployeeRepository implements EmployeeRepositoryInterface
{
    ...
    {
        $stmt = $this->db->prepare("SELECT id, nome, setor_id, criado_em, atualizado_em FROM funcionarios WHERE id = ? AND deletado_em IS NULL");
        $stmt->execute([$id]);
    ...
    {
        $stmt = $this->db->query("SELECT id, nome, setor_id, criado_em, atualizado_em FROM funcionarios WHERE deletado_em IS NULL ORDER BY nome ASC");
        $employees = [];
    ...
    {
        $stmt = $this->db->prepare("SELECT id, nome, setor_id, criado_em, atualizado_em FROM funcionarios WHERE setor_id = ? AND deletado_em IS NULL");
    ...
    {
        $sql = "SELECT COUNT(*) FROM funcionarios WHERE status = 'ATIVO' AND deletado_em IS NULL";
        $params = [];
    ...
    {
        $stmt = $this->db->prepare("UPDATE funcionarios SET deletado_em = CURRENT_TIMESTAMP WHERE id = ?");
