...
class MySQLDepartmentRepository implements DepartmentRepositoryInterface
{
    ...
    {
        $result = $this->db->query("SELECT id, nome, nome_en, sigla, status, criado_em, atualizado_em FROM setores ORDER BY nome ASC");
        $departments = [];
    ...
    public function save(Department $department): void
    {
        ...
        $stmt->bind_param('ssss', $nome, $nomeEn, $sigla, $episJson);
        $stmt->execute();
    ...
    public function update(Department $department): void
    {
        ...
        $episJson = json_encode($department->getEpis());
        $id = $department->getId();
        $stmt->bind_param('ssssi', $nome, $nomeEn, $sigla, $episJson, $id);
        $stmt->execute();
    }
