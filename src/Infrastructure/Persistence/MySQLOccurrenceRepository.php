...
class MySQLOccurrenceRepository implements OccurrenceRepositoryInterface
{
    ...
    public function findById(int $id): ?Occurrence
    {
        ...
        $stmt = $this->db->prepare("SELECT * FROM ocorrencias WHERE id = ? AND filial_id = ?");
    ...
    {
        $activeFilial = $_SESSION['active_filial_id'] ?? 1;
        $result = $this->db->query("SELECT * FROM ocorrencias WHERE filial_id = $activeFilial ORDER BY data_hora DESC");
        $occurrences = [];
    ...
    {
        $activeFilial = $_SESSION['active_filial_id'] ?? 1;
        $stmt = $this->db->prepare("SELECT * FROM ocorrencias WHERE funcionario_id = ? AND filial_id = ? ORDER BY data_hora DESC");
    ...
    public function countDaily(DateTimeInterface $date, ?array $sectorIds = null): int
    {
        ...
        if (!empty($sectorIds)) {
            $query .= " AND o.filial_id = ?";
            $types = 's' . str_repeat('i', count($sectorIds)) . 'i';
            $params = array_merge([$dateStr], $sectorIds, [$activeFilial]);
            $stmt = $this->db->prepare($query);
            $stmt->bind_param($types, ...$params);
        } else {
            $query .= " AND o.filial_id = ?";
            $stmt = $this->db->prepare($query);
            $stmt->bind_param('si', $dateStr, $activeFilial);
        }
    ...
    public function countMonthly(DateTimeInterface $date, ?array $sectorIds = null): int
    {
        ...
        if (!empty($sectorIds)) {
            $query .= " AND o.filial_id = ?";
            $types = 'ss' . str_repeat('i', count($sectorIds)) . 'i';
            $params = array_merge([$dateStr, $dateStr], $sectorIds, [$activeFilial]);
            $stmt = $this->db->prepare($query);
            $stmt->bind_param($types, ...$params);
        } else {
            $query .= " AND o.filial_id = ?";
            $stmt = $this->db->prepare($query);
            $stmt->bind_param('ssi', $dateStr, $dateStr, $activeFilial);
        }
    ...
    public function getMonthlyInfractionStats(int $year, ?array $sectorIds = null): array
    {
        ...
        $query = "
            ...
            WHERE YEAR(o.data_hora) = ? AND o.tipo = 'INFRACAO' AND o.filial_id = ?
            $sectorClause
            $epiClause
        ...
        // Re-construct types and params for final query to include filial_id
        $finalTypes = "ii" . (str_replace('i', '', $epiTypes)); // Start with year, filial then append suffixes
        // Wait, epiTypes has 'i' for year. Let's rebuild properly.
        $finalTypes = "ii";
        $finalParams = [$year, $activeFilial];
        if (!empty($sectorIds)) {
            $finalTypes .= str_repeat('i', count($sectorIds));
            $finalParams = array_merge($finalParams, $sectorIds);
        }
        if (!empty($allowedEpiNames)) {
            $finalTypes .= str_repeat('s', count($allowedEpiNames));
            $finalParams = array_merge($finalParams, $allowedEpiNames);
        }
    ...
    {
        $activeFilial = $_SESSION['active_filial_id'] ?? 1;
        $stmt = $this->db->prepare("INSERT INTO ocorrencias (funcionario_id, tipo, data_hora, filial_id) VALUES (?, ?, ?, ?)");
        $fid = $occurrence->getEmployee()->getId();
        $tipo = $occurrence->getType()->getValue();
        $data = $occurrence->getDate()->format('Y-m-d H:i:s');
        $stmt->bind_param('issi', $fid, $tipo, $data, $activeFilial);
        $stmt->execute();
    ...
    public function findInfractions(array $filters = []): array
    {
        ...
        if (!$stmt) {
            error_log("Prepare failed: " . $this->db->error . " with SQL: " . $sql);
            return [];
        }
    ...
    public function findNewInfractions(int $lastId): array
    {
        ...
        $sql = "SELECT o.id, f.nome as funcionario_nome, s.sigla as setor_sigla, e.nome as epi_nome, o.data_hora
                ...
                LEFT JOIN ocorrencia_epis oe ON o.id = oe.ocorrencia_id
                LEFT JOIN epis e ON oe.epi_id = e.id
                WHERE {$where} AND o.tipo = 'INFRACAO' AND o.oculto = FALSE AND o.filial_id = ?
                {$orderLimit}";
        ...
        if ($lastId !== -1) {
            $stmt->bind_param('ii', $lastId, $activeFilial);
        } else {
            $stmt->bind_param('i', $activeFilial);
        }
