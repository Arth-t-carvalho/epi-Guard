...
class MySQLAuditLogRepository implements AuditLogRepositoryInterface
{
    ...
    {
        $stmt = $this->db->query("SELECT * FROM audit_logs ORDER BY criado_em DESC");
        $logs = [];
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM audit_logs WHERE usuario_id = ? ORDER BY criado_em DESC");
        $stmt->execute([$userId]);
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM audit_logs WHERE tipo_entidade = ? AND entidade_id = ? ORDER BY criado_em DESC");
